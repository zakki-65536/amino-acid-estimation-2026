...
def main(hpo_depth,hpo_learning_rate,hpo_l2_leaf_reg,hpo_num_leaves):
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_cb), start=1):
        ...
        cb = CatBoostRegressor(
            ...
            learning_rate=hpo_learning_rate,  # 1本の木がどれだけ強く補正するか 0.03はやや学習が遅め
            depth=hpo_depth,  # 1本の木の深さの最大 深いほど過学習しやすい
            l2_leaf_reg=hpo_l2_leaf_reg,  # L2正則化（過学習抑制）
        ...
        lgbm = lgb.LGBMRegressor(
            ...
            num_leaves=hpo_num_leaves,  # 1本の木の最大葉数
    ...
    timestamp_console=datetime.now().strftime("%m/%d %H:%M:%S ")+str(hpo_depth)+" "+str(hpo_learning_rate)+" "+str(hpo_l2_leaf_reg)+" "+str(hpo_num_leaves)
    ...
    # ---- CatBoost ----
    cat_file = "result_catboost_hpo.xlsx"
    ...
    # ---- LightGBM ----
    lgb_file = "result_lightgbm_hpo.xlsx"
