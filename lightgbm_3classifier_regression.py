...
def run_experiment(
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_cb, y), start=1):
        ...
        pred_lgb = lgbm.predict(
            X_va_lgb,
            num_iteration=lgbm.best_iteration_
        )
        # pred_label_lgb = np.rint(pred_lgb).astype(int)
        # pred_label_lgb = np.clip(pred_label_lgb, 1, 3)

        def to_label_by_threshold(pred, thresholds):
            t1, t2 = thresholds
            return np.digitize(pred, bins=[t1, t2]) + 1
            # pred <= t1 -> 1
            # t1 < pred <= t2 -> 2
            # t2 < pred -> 3

        def objective_threshold(thresholds, y_true, pred):
            t1, t2 = thresholds
            ...
            y_pred = to_label_by_threshold(pred, thresholds)

            # 順序重視なら MAE を最小化
            return mean_absolute_error(y_true, y_pred)

        def objective_threshold_mae_f1(thresholds, y_true, pred, alpha=0.9):
            t1, t2 = thresholds
            ...
            return alpha * mae + (1 - alpha) * (1 - f1)

        # validation予測 pred_lgb に対して閾値最適化
        res = minimize(
            objective_threshold_mae_f1,
            x0=[1.5, 2.5],
            args=(y_va, pred_lgb),
            method="Nelder-Mead"
        )
        ...
        pred_label_lgb = to_label_by_threshold(pred_lgb, best_thresholds)
        ...
        # print(
        #     df_check.groupby("y_true")["pred"]
        #     .agg(["count", "mean", "std", "min", "median", "max"])
        # )

        lgb_all_pred_list.append(pred_label_lgb)
    ...
    error = lgb_all_pred - all_true
    print(pd.Series(error).value_counts().sort_index())
    print(pd.Series(error).value_counts(normalize=True).sort_index())

    # 完全一致Accuracy
    # cb_acc = accuracy_score(all_true, cb_all_pred)
    lgb_acc = accuracy_score(all_true, lgb_all_pred)

    # F1
    # cb_f1_macro = f1_score(all_true, cb_all_pred, average="macro")
    lgb_f1_macro = f1_score(all_true, lgb_all_pred, average="macro")

    # cb_f1_weighted = f1_score(all_true, cb_all_pred, average="weighted")
    lgb_f1_weighted = f1_score(all_true, lgb_all_pred, average="weighted")

    # 誤差系
    # cb_mae = mean_absolute_error(all_true, cb_all_pred)
    lgb_mae = mean_absolute_error(all_true, lgb_all_pred)

    # cb_rmse = mean_squared_error(all_true, cb_all_pred, squared=False)
    # lgb_rmse = mean_squared_error(all_true, lgb_all_pred, squared=False)

    # ±1以内正解率
    # cb_acc_pm1 = np.mean(np.abs(all_true - cb_all_pred) <= 1)
    lgb_acc_pm1 = np.mean(np.abs(all_true - lgb_all_pred) <= 1)

    # ±2以内正解率
    # cb_acc_pm2 = np.mean(np.abs(all_true - cb_all_pred) <= 2)
    ...
    cm = confusion_matrix(all_true, lgb_all_pred, labels=labels)
